		...
		if (m_IDirect3DDevice9)
		{
			...
			if (SUCCEEDED(m_IDirect3DDevice9->BeginScene()))
			{
				if (m_game == GetForegroundWindow())
				{
					...
					static bool state = m_cheat.get_process();
					static DWORD client = m_cheat.find_module("client.dll");
					static DWORD engine = m_cheat.find_module("engine.dll");
					assert(state && client && engine && "获取CSGO信息失败");

					do
					{
						RECT rect{ 0 };
						GetClientRect(m_game, &rect);
						int half_width = (rect.right - rect.left) / 2;
						int half_height = (rect.bottom - rect.top) / 2;

						DWORD local = m_cheat.read<DWORD>(client + dwLocalPlayer);
						if (local == 0) break;

						int health = m_cheat.read<int>(local + m_iHealth);
						if (health <= 0) break;

						struct help { float data[4][4]{ 0 }; };
						help matrix = m_cheat.read<help>(client + dwViewMatrix);

						for (int i = 0; i < 64; i++)
						{
							DWORD entity = m_cheat.read<DWORD>(client + dwEntityList + i * 0x10);
							if (entity == 0 || entity == local) continue;

							if (m_cheat.read<int>(entity + m_iHealth) <= 0) continue;

							D3DXVECTOR3 bone = m_cheat.get_bone_pos(entity, 8);
							if (bone == D3DXVECTOR3{ 0,0,0 }) continue;

							int x, y, w, h;
							if (m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, x, y, w, h) == false) continue;

							//头部
							render_line(half_width, 0, x, y, D3DCOLOR_ARGB(255, 255, 0, 255));
							//render_line(half_width, half_height, x, y);

							bone = m_cheat.get_bone_pos(entity, 10);
							int x1, y1;
							m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, x1, y1, w, h);

							//胸部
							render_line(x, y, x1, y1);

							//右手
							{
								int bones[3][2]{ 0 };

								bone = m_cheat.get_bone_pos(entity, 11);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[0][0], bones[0][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 12);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[1][0], bones[1][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 13);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[2][0], bones[2][1], w, h);

								for (int i = 0; i < 2; i++) render_line(bones[i][0], bones[i][1], bones[i + 1][0], bones[i + 1][1]);

								render_line(bones[0][0], bones[0][1], x1, y1);
							}
							...
							//左手
							{
								...
								bone = m_cheat.get_bone_pos(entity, 39);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[0][0], bones[0][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 40);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[1][0], bones[1][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 41);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[2][0], bones[2][1], w, h);
							...
							bone = m_cheat.get_bone_pos(entity, 3);
							m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, x, y, w, h);
							...
							//左腿
							{
								...
								bone = m_cheat.get_bone_pos(entity, 66);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[0][0], bones[0][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 67);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[1][0], bones[1][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 68);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[2][0], bones[2][1], w, h);

								for (int i = 0; i < 2; i++) render_line(bones[i][0], bones[i][1], bones[i + 1][0], bones[i + 1][1]);

								render_line(bones[0][0], bones[0][1], x, y);
							}
							...
							//右腿
							{
								...
								bone = m_cheat.get_bone_pos(entity, 74);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[0][0], bones[0][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 75);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[1][0], bones[1][1], w, h);

								bone = m_cheat.get_bone_pos(entity, 76);
								m_cheat.to_rect_info(matrix.data, bone, half_width, half_height, bones[2][0], bones[2][1], w, h);
